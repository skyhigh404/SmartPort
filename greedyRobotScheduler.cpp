...
{
    // 机器人没携带货物，并且没有目标
    if (robot.carryingItem == 0 && robot.targetid == -1) return true;
    return false;
}
...
{
    // 机器人携带货物，并且没有目标
    if (robot.carryingItem == 1 && robot.carryingItemId != -1 && robot.targetid == -1) return true;
    return false;
}
                                       ...
                                       const Map &map)
{
    ...
    for (int j=0;j<availableGoods.size();j++) {
        ...
        if (berthid==-1) {
            ...
            if (!canReach) cost_robot2good[j] = INT_MAX;
            else cost_robot2good[j] = map.cost(robot.pos, availableGoods[j].get().pos);
        }
        else cost_robot2good[j] = map.berthDistanceMap.at(berthid)[availableGoods[j].get().pos.x][availableGoods[j].get().pos.y];
    }
                         ...
                         vector<long long>& cost_good2berth)
{
    ...
    for (int j = 0; j < availableGoods.size(); j++) {
        if (cost_robot2good[j] >= INT_MAX || cost_good2berth[j] >= INT_MAX) continue;
        profits[j] = availableGoods[j].get().value*1.0 / (cost_robot2good[j] + cost_good2berth[j]);

        if (availableGoods[j].get().TTL<=TTL_Bound && !enterFinal) profits[j] *= TTL_ProfitWeight;
    }
    // 对收益排序
    std::vector<int> indices(availableGoods.size(), 0);
    for (int j=0;j<availableGoods.size();j++) indices[j] = j;
...
void
GreedyRobotScheduler::findBerthForRobot(Robot &robot,
                                        std::vector<Goods> &goods,
                                        const std::vector<Berth> &berths,
                                        const Map& map)
{
